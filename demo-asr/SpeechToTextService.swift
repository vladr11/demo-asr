...
//  Readability
//
//  Created by Vlad Rusu on 29/06/2020.
...
//        let speechRecognitionRequest: SFSpeechAudioBufferRecognitionRequest
    ...
    private let workingQueue = DispatchQueue(label: "com.readability.SpeechToTextService",
                                             qos: .userInitiated,
...
extension SpeechToTextService {
    ...
    private func prepare_internal() throws {
        ...
        let inputFormat = inputNode.outputFormat(forBus: .zero)
        
        inputNode.installTap(onBus: .zero, bufferSize: 4096, format: inputFormat) { (audioBuffer, when) in
